...
class GUI:
    ...
    def drawPlayer(self, screenObj, playerObj):
        screen = screenObj.screen
        pygame.draw.line(screen, RED, (playerObj.x_player-self.lineWidth, playerObj.y_player),
                         (playerObj.x_player+self.lineWidth, playerObj.y_player), self.lineWidth)
    ...
    def createTextBox(self, text, textColor, boxColor, margin_x, margin_y):
        ...
        boxSurf = pygame.Surface(
            textSurf.get_rect().inflate(margin_x, margin_y).size)
        boxSurf.fill(boxColor)
        boxSurf.blit(textSurf, textSurf.get_rect(
            center=boxSurf.get_rect().center))
        return boxSurf
    ...
    def drawRowInput(self, screenObj, inputObj):
        ...
        if (inputObj.isRowInputFocus == True):
            textSurf = self.createTextBox(
                f'MAZE SIZE: {inputObj.rowInput}|', BLACK, PINK, 10, 5)
        else:
            textSurf = self.createTextBox(
                f'MAZE SIZE: {inputObj.rowInput}', BLACK, PINK, 10, 5)
    ...
    def drawDifficultyInput(self, screenObj, inputObj):
        ...
        if (inputObj.isDifficultyInputFocus == True):
            textSurf = self.createTextBox(
                f'DIFFICULTY: {inputObj.difficultyInput}|', BLACK, PINK, 19, 5)
        else:
            textSurf = self.createTextBox(
                f'DIFFICULTY: {inputObj.difficultyInput}', BLACK, PINK, 19, 5)
    ...
    def drawSolutionPath(self, solversObj, gameObj):
